...
1. Verify Figma MCP is connected (it's a deferred tool — call `ToolSearch` with query "figma" to load its schemas)
2. Look for the `outsystems-conventions` memory → if absent, invoke `outsystems-onboarding`
3. Look for the `outsystems-project-context` memory → if absent, invoke `outsystems-project-context`
...
Load these with `ToolSearch`, then call them. Batch every tool you expect to need into ONE `ToolSearch` call — the `select:` query takes a comma-separated list. **Always read the tool descriptions `ToolSearch` returns before calling** — parameter names matter.
