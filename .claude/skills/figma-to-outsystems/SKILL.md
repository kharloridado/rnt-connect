...
Look for the `outsystems-conventions` memory — recalled memories arrive automatically in `<system-reminder>` blocks, and `MEMORY.md` indexes them. If it's absent → invoke `outsystems-onboarding` skill.
...
Look for the `outsystems-project-context` memory. If it's absent → invoke `outsystems-project-context` skill.
...
Check the project's `CLAUDE.md` / `project-context.md` (or the `outsystems-project-context` memory) for:
